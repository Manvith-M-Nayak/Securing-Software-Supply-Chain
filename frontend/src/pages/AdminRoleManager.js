...
function AdminRoleManager({ user: parentUser }) {
  ...
  const fetchProjectsAndUsers = useCallback(async () => {
    if (!user?.githubUsername) {
      ...
      setStatus('❌ User GitHub username missing.');
      return;
    }

    // 1) Fetch projects created by admin
    try {
      console.log(`Fetching projects from: http://localhost:5001/admin/projects/${user.githubUsername}`);
      const projectRes = await fetch(`http://localhost:5001/admin/projects/${user.githubUsername}`);
      if (!projectRes.ok) {
        let errorMessage = `HTTP ${projectRes.status}: ${projectRes.statusText}`;
        try {
    ...
    try {
      ...
      if (!userRes.ok) {
        let errorMessage = `HTTP ${userRes.status}: ${userRes.statusText}`;
        try {
